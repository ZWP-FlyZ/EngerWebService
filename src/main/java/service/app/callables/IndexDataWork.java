...
public class IndexDataWork extends DataAuthBaseWork {

	
	private final static Logger logger = LoggerFactory.getLogger(IndexDataWork.class);
	public IndexDataWork(RequestData data) {
		super(data);
...
//			}
			...
			for(AllSimData d:allData){
				...
				engMonMap.put(d.getFuelType(),tmp,td);
			}
			...
			if(portData!=null){
				for(PortProData d:portData){
					...
					tmp = TimeTools.getYearMonth(d.getInTime());
					if(tmp!=null){
						td = engMonMap.get(TypeGetter.FT_CHAI_YOU,tmp);//柴油
						if(td==null)
						{
							td = new TypeData();
							td.setType(tmp);
						...
						engMonMap.put(TypeGetter.FT_CHAI_YOU,tmp,td);
						
						td = engMonMap.get(TypeGetter.FT_QI_YOU,tmp);
						if(td==null)
						{
							td = new TypeData();
							td.setType(tmp);
						...
						engMonMap.put(TypeGetter.FT_QI_YOU,tmp,td);
						
						td = engMonMap.get(TypeGetter.FT_MEI_YOU,tmp);
						if(td==null)
						{
							td = new TypeData();
							td.setType(tmp);
						...
						engMonMap.put(TypeGetter.FT_MEI_YOU,tmp,td);
						
						td = engMonMap.get(TypeGetter.FT_DIAN_NENG,tmp);
						if(td==null)
						{
							td = new TypeData();
							td.setType(tmp);
						...
						engMonMap.put(TypeGetter.FT_DIAN_NENG,tmp,td);
						
						td = engMonMap.get(TypeGetter.FT_QI_TA,tmp);
						if(td==null)
						{
							td = new TypeData();
							td.setType(tmp);
						...
						engMonMap.put(TypeGetter.FT_QI_TA,tmp,td);
